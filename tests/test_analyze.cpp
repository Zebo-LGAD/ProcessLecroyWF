...
std::string gScopeDataFile = gScopeDataFolder + "../ProcessedROOTFiles/AC-W1-190V-20C.root";
std::map<int, double> gChTimeOffsets;
void NormalizeTOA(WFDataProcessor::WFDataTreeReader &reader)
{
    gSystem->mkdir("plots/NormalizeTOA/", true);
    ...
    for (auto &iter : mhTOA)
    {
        ...
        cTOA.SaveAs(("plots/NormalizeTOA/Ch" + std::to_string(ch) + "_TOA.png").c_str());
    }
}
...
int main()
{
    ...
    hitXCor->Draw("colz");
    c->SaveAs("plots/hitXCor.png");

    hxPos->Draw();
    c->SaveAs("plots/hxPos.png");
    hxMeasure->Draw();
    c->SaveAs("plots/hxMeasure.png");

    hdeltaTx->Draw("colz");
    c->SaveAs("plots/hdeltaTx.png");
    auto hpfx = hdeltaTx->ProfileX();
    hpfx->SetLineColor(kRed);
    hpfx->Draw("same");
    c->SaveAs("plots/hdeltaTxProfileX.png");
    hdeltaTx->FitSlicesY();
    auto hfpy = (TH1D *)gDirectory->Get("hdeltaTx_2");
    hfpy->SetLineColor(kBlue);
    hfpy->Draw("hist");
    // hfpy->GetYaxis()->SetRangeUser(0, 0.1);
    c->SaveAs("plots/hdeltaTxFitSlicesY.png");
    ...
    for (int bin = 1; bin <= hfpy->GetNbinsX(); bin++)
    {
        ...
        c->SaveAs(Form("plots/hdeltaTx_ProjBin%d.png", bin));
    }
    hdeltaTxSigmat->Draw("hist");
    hdeltaTxSigmat->GetYaxis()->SetRangeUser(0, 100);
    c->SaveAs("plots/hdeltaTx_TimeResolution.png");
    ...
    for (int bin = 1; bin <= hfpy->GetNbinsX(); bin++)
    {
        ...
        c->SaveAs(Form("plots/hdeltaTx2_ProjBin%d.png", bin));
    }
    ...
    c->SaveAs("plots/hdeltaTx2_TimeResolution.png");
    ...
    c->SaveAs("plots/hdeltaTx2_TimeResolution_ProjY.png");
